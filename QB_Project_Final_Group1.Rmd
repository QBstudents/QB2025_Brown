...
region_colors <- colorFactor(palette = "Set1", domain = fish_data$Lat_Zone)

leaflet(data = fish_data) %>%
  addTiles() %>%
  addCircleMarkers(lng = lons, lat = lats, 
                   color = ~region_colors(Lat_Zone),
                   radius = 5, fillOpacity = 0.8,
                   popup = ~paste("Region:", Lat_Zone, "<br>Lat:", lats, "<br>Lon:", lons)) %>%
  addLegend("bottomright", pal = region_colors, values = fish_data$Lat_Zone,
            title = "Regions")
